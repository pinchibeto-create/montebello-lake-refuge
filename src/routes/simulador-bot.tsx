...
type BotResult={reply:string;trace?:Trace[];configured?:boolean;error?:string};
...
function BotSimulator(){
 ...
 const [input,setInput]=useState(""); const [trace,setTrace]=useState<Trace[]>([]); const [busy,setBusy]=useState(false); const [error,setError]=useState("");
 async function simulate(text:string){
  if(!text.trim()||busy)return; setMessages(m=>[...m,{role:"cliente",text}]);setBusy(true);setError("");setTrace([]);
  try{const session=getStoredSession();if(!session)throw new Error("Primero inicia sesión en /panel y después vuelve al simulador.");
   ...
   setTrace(result.trace||[]);setMessages(m=>[...m,{role:"bot",text:result.reply||result.error||"Sin respuesta"}]);
 ...
 return <main style={{minHeight:"100vh",background:"#f4f1e8",padding:"28px 16px",color:"#173326",fontFamily:"system-ui,sans-serif"}}><div style={{maxWidth:1180,margin:"0 auto"}}>
  ...
  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.55fr) minmax(280px,.8fr)",gap:18}}>
   <section style={{background:"white",borderRadius:20,overflow:"hidden",boxShadow:"0 12px 35px #17332618"}}><div style={{padding:14,background:"#173326",color:"white",fontWeight:750}}>WhatsApp simulado <span style={{float:"right",fontSize:12,opacity:.7}}>{busy?"PENSANDO…":"IA REAL · PRUEBA"}</span></div>
   ...
   <aside style={{display:"grid",gap:14,alignContent:"start"}}>{error&&<Card title="Error"><div style={{fontSize:13}}>{error}</div></Card>}<Card title="Herramientas usadas">{trace.length?trace.map((x,i)=><div key={i} style={{padding:"8px 0",borderBottom:"1px solid #eee"}}><b style={{fontFamily:"monospace",fontSize:12}}>{x.tool}</b><pre style={{fontSize:10,whiteSpace:"pre-wrap",maxHeight:130,overflow:"auto"}}>{JSON.stringify({args:x.args,result:x.result},null,2)}</pre></div>):<div style={{fontSize:13,opacity:.65}}>Aquí aparecerán las consultas reales que haga la IA.</div>}</Card>
    ...
    <button onClick={()=>{setMessages([]);setTrace([]);setError("")}} style={{border:"1px solid #173326",borderRadius:12,padding:11,background:"transparent",fontWeight:750}}>Reiniciar prueba</button>
   </aside>
  </div></div></main>
}
