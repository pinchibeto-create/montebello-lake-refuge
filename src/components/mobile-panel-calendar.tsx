...
const shortGuestName=(name:string|null)=>{if(!name)return "Reserva";const parts=name.trim().split(/\s+/).filter(Boolean);return parts.slice(0,2).join(" ")||"Reserva"};
...
export default function MobilePanelCalendar(){
 ...
 useEffect(()=>{if(!client||!session)return;(async()=>{setLoading(true);try{const from=iso(new Date(month.getFullYear(),month.getMonth(),1));const to=iso(new Date(month.getFullYear(),month.getMonth()+1,8));const rs=await api<Reservation[]>(`reservations?select=id,cabin_id,source,guest_name,guest_phone,check_in,check_out,payment_status,reservation_code,total_amount,paid_amount,guests,payment_reference&cliente_id=eq.${client}&status=in.(pendiente,confirmada)&check_in=lt.${to}&check_out=gt.${from}`,session);setReservations(rs)}finally{setLoading(false)}})()},[client,session,month]);
 const week=useMemo(()=>Array.from({length:7},(_,i)=>add(selected,i)),[selected]);
  ...
  <main className="p-3">
   ...
   <section className="mt-3 overflow-auto rounded-2xl border bg-white shadow-sm" style={{maxHeight:"calc(100vh - 330px)"}}>
    <div className="min-w-[790px]">
     <div className="sticky top-0 z-30 grid grid-cols-[84px_repeat(7,101px)] border-b bg-[#f8f6f1] text-center"><div className="sticky left-0 z-40 bg-[#f8f6f1] px-1 py-2 text-[9px] font-bold uppercase">Cabaña</div>{week.map(d=><div key={iso(d)} className="border-l px-1 py-2"><div className="text-[9px] font-bold text-[#1f8f7a]">{dayName(d)}</div><div className="text-[11px] font-semibold">{d.getDate()}/{d.getMonth()+1}</div></div>)}</div>
     {loading?<div className="p-5 text-center text-xs">Cargando…</div>:cabins.map(c=><div key={c.id} className="grid grid-cols-[84px_repeat(7,101px)] border-b last:border-0"><div className="sticky left-0 z-20 flex min-h-[54px] items-center bg-white px-2 text-[10px] font-bold shadow-[2px_0_3px_rgba(0,0,0,.05)]">{c.nombre}</div>{week.map(d=>{const r=occupied(reservations,c.id,iso(d));return <div key={iso(d)} className="border-l p-1">{r?<button onClick={()=>setDetail(r)} title={r.guest_name||"Reservación"} className={`h-full min-h-[46px] w-full rounded-lg p-1 text-left ${r.payment_status==="pagado"?"bg-emerald-100":r.payment_status==="anticipo"?"bg-amber-100":"bg-red-50"}`}><div className="truncate text-[9px] font-bold">{shortGuestName(r.guest_name)}</div><div className="mt-0.5 truncate text-[8px] uppercase">{r.source}</div></button>:<div className="flex min-h-[46px] items-center justify-center rounded-lg bg-emerald-50 text-[9px] font-semibold text-emerald-700">Libre</div>}</div>})}</div>)}
    </div>
   </section>
  </main>
